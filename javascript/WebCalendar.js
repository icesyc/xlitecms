...
function SelectDate(obj,strFormat)
{
    ...
    cal.dateFormatStyle = strFormat || 'yyyy-MM-dd';
    cal.show(obj);
}
/**//**//**//**//**//**//**//**
 * 返回日期
...
function Calendar(beginYear, endYear, lang, dateFormatStyle) {
  ...
  "tr_bg"         : "#666666",  //日历头背影色
  "input_border"  : "#FFFFFF",  //input控件的边框颜色
...
Calendar.prototype.draw = function() {
  ...
  mvAry[mvAry.length]  = '        <th align="left" width="10%"><input style="border: 0 solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:20px;height:20px;font-weight:bold;" name="prevMonth" type="button" id="prevMonth" value="&lt;" /></th>';
  mvAry[mvAry.length]  = '        <th align="center" width="80%" nowrap="nowrap"><select name="calendarYear" id="calendarYear" style="font-size:12px;"></select><select name="calendarMonth" id="calendarMonth" style="font-size:12px;"></select></th>';
  mvAry[mvAry.length]  = '        <th align="right" width="10%"><input style="border: 0 solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:20px;height:20px;font-weight:bold;" name="nextMonth" type="button" id="nextMonth" value="&gt;" /></th>';
  mvAry[mvAry.length]  = '      </tr>';
  mvAry[mvAry.length]  = '    </table>';
  mvAry[mvAry.length]  = '    <table id="calendarTable" width="100%" style="background-color:#FFFFFF" border="0" cellpadding="3" cellspacing="0">';
  mvAry[mvAry.length]  = '      <tr>';
  for(var i = 0; i < 7; i++) {
  ...
  mvAry[mvAry.length]  = '      </tr>';
  for(var i = 0; i < 6;i++){
    mvAry[mvAry.length]  = '    <tr align="center" style="font-size:11px;font-family:verdana">';
    for(var j = 0; j < 7; j++) {
      if (j == 0){
  ...
  mvAry[mvAry.length]  = '        <th colspan="2" style="border-top:1px solid #ddd"><input name="calendarClear" type="button" id="calendarClear" value="' + Calendar.language["clear"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:100%;height:20px;font-size:12px;"/></th>';
  mvAry[mvAry.length]  = '        <th colspan="3" style="border-top:1px solid #ddd"><input name="calendarToday" type="button" id="calendarToday" value="' + Calendar.language["today"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:100%;height:20px;font-size:12px;"/></th>';
  mvAry[mvAry.length]  = '        <th colspan="2" style="border-top:1px solid #ddd"><input name="calendarClose" type="button" id="calendarClose" value="' + Calendar.language["close"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:100%;height:20px;font-size:12px;"/></th>';
  mvAry[mvAry.length]  = '      </tr>';
  mvAry[mvAry.length]  = '    </table>';
